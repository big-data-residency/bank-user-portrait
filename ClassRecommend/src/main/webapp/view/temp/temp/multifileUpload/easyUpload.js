    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += '<span class="easy_upload_select noselect btn-default">' + option.selectText + '</span>';
          $html += option.multi ? '<span class="easy_upload_head_btn1 noselect btn-default">上传</span>' : '';
          $html += option.multi ? '<span class="easy_upload_head_btn2 noselect btn-default">删除</span>' : '';
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += file.allow ? '<p class="easy_upload_upbtn btn noselect btn-default">上传</p>' : '';
            $html += '<p class="easy_upload_delbtn btn noselect btn-default">删除</p>';
            $html += '</div>';
        ...
        _handleCheck: function (opt) {
          ...
          if (opt.type === 'all') {
            if (opt.check === 'yes') {
              ...
              var qItems = $(opt.target).parent().parent().parent().find('.queue_check');
              for (var i=0; i<qItems.length; i++) {
                $(qItems[i]).empty().html('&#xe61e;').attr('data-checked', 'yes');
              }
            } else {
              $(opt.target).empty().html('&#xe693;').attr('data-checked', 'no');
              var qItems = $(opt.target).parent().parent().parent().find('.queue_check');
              for (var i=0; i<qItems.length; i++) {
                $(qItems[i]).empty().html('&#xe693;').attr('data-checked', 'no');
            }
            }
          } else {
            if (opt.check === 'yes') {
